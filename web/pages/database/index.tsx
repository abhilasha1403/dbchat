...
export function isBigQueryDb(dbTypeList: DBOption[], dbType: DBType) {
  debugger;
  return dbTypeList.find((item) => item.value === dbType)?.isBigQueryDb;
}
...
function Database() {
  ...
  const dbTypeList = useMemo(() => {
    const supportDbList = dbSupportList.map((item) => {
      const { db_type, is_file_db,is_bigquery_db } = item;

      return { ...dbMapper[db_type], value: db_type, isFileDb: is_file_db, isBigQueryDb:is_bigquery_db };
    }) as DBOption[];
