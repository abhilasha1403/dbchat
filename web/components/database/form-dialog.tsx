...
import { isFileDb,isBigQueryDb } from '@/pages/database';
...
function FormDialog({ open, choiceDBType, dbTypeList, editValue, dbNames, onClose, onSuccess }: Props) {
  ...
  const bigQueryDb = useMemo(() => isBigQueryDb(dbTypeList, dbType), [dbTypeList, dbType]);
  useEffect(() => {
    if (choiceDBType) {
    ...
    <Modal open={open} width={400} title={editValue ? t('Edit') : t('create_database')} maskClosable={false} footer={null} onCancel={onClose}>
      <Form form={form} className="pt-2" labelCol={{ span: 6 }} labelAlign="left" onFinish={onFinish}>
        ...
        {fileDb === true && (
         ...
         {fileDb === false && bigQueryDb === true && (
          <>
            <Form.Item name="db_host" label="Project" className="mb-3" rules={[{ required: true }]}>
            <Input />
            </Form.Item>
            <Form.Item name="db_pwd" label="Credentials" className="mb-3" rules={[{ required: true }]}>
            <Input type="password" />
          </Form.Item>
          </>
        )}
        {fileDb === false && bigQueryDb==false && (
          <>
