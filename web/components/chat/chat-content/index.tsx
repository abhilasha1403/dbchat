...
const pluginViewStatusMapper: Record<CHATView['status'], { bgClass: string; icon: ReactNode }> = {
  todo: {
    bgClass: 'bg-gray-500',
...
function ChatContent({ children, content, isChartChat, onLinkClick }: PropsWithChildren<Props>) {
  ...
  const { relations, value, cachePluginContext } = useMemo<{ relations: string[]; value: string; cachePluginContext: CHATView[] }>(() => {
    ...
    const cachePluginContext: CHATView[] = [];

    let cacheIndex = 0;
    const result = value.replace(/<chat-view[^>]*>[^<]*<\/chat-view>/gi, (matchVal) => {
      try {
        const pluginVal = matchVal.replaceAll('\n', '\\n').replace(/<[^>]*>|<\/[^>]*>/gm, '');
        const pluginContext = JSON.parse(pluginVal) as CHATView;
